...
Error String_print(const String* string_obj_p)
{
...
Error String_println(const String* string_obj_p)
{
    Error result = String_print(string_obj_p);
...
void test_string()
{
    ...
    ASSERT_EQ(
        (int)String_println(test_string_p).err.code, ERR_ALL_GOOD, "Printing functions work.");
    ...
    ASSERT_EQ(
        String_starts_with(test_string_p, ""), true, "starts_with() works when needle is empty");
    String_destroy(test_string_p);
    ...
    num_of_replacements = unwrap(String_replace_pattern(test_string_p, "\\r\\n", "HELLO WORLD"));
    ASSERT_EQ(
        "This pattern contains HELLO WORLD to be replaced", test_string_p->str, "Pattern replaced");
    ASSERT_EQ(num_of_replacements, 1, "Number of replacements counted correctly.");
    String_destroy(test_string_p);
